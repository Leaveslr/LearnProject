...
一个真正让小白学会的视频，不是平均分配“概念、例子、练习题”，而是：

```text
最小概念地图 -> 单一贯穿例子 -> 机制拆解 -> 关键细节 -> 判断题验证
```

推荐比例：

- 概念地图：15%-20%。只讲这个知识点解决什么问题、输入输出是什么、为什么需要它。
- 贯穿例子：55%-65%。只用一个例子从头走到尾，每个抽象术语都回到同一个例子上。
- 判断练习：15%-20%。不做记忆题，而做判断题，让用户证明自己知道“为什么这么做”。
...
| 2. 最小概念 | 给地图，不堆术语 | 它的输入、输出、目标是什么？ |
| 3. 单一例子 | 固定心智锚点 | 我们用哪个例子贯穿到底？ |
| 4. 机制拆解 | 一步一步走例子 | 这个例子里的每一步发生了什么？ |
| 5. 关键细节 | 补真正会卡住的地方 | 公式、边界、实现细节分别是什么意思？ |
| 6. 反事实/对比 | 解释为什么这样做 | 如果不这样做，会错在哪里？ |
| 7. 判断题 | 验证是否真懂 | 用户能否判断一个说法为什么对或错？ |
| 8. 三句总结 | 收束成可复述心智模型 | 用户离开视频后能记住哪三句话？ |

### 判断题设计

判断题必须检查理解，而不是背诵。合格判断题通常长这样：

- 如果正确 token 概率升高，loss 会怎么变，为什么？
- 如果 mask 错了，模型会学到什么错误目标？
- 如果 PPL 低，为什么仍然可能任务失败？
- 如果把某个模块去掉，训练或推理会坏在哪里？

如果用户不能回答这些题，说明视频还只是“讲过了”，没有“讲懂”。
...
- 讲解逻辑 20 分：问题先行，术语后置
- 单例子贯穿 20 分：同一个例子能走完整条机制链
- 准确完整 20 分：机制、公式、边界不缺
- 判断力 20 分：用户能做对“为什么”判断题
