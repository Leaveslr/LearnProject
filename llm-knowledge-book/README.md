...
- **基础原理拆分**：优化器、损失函数、归一化、Embedding、采样策略。
- **训练技术深化**：DPO、GRPO/GSPO、数据工程、分布式训练。
- **推理优化深化**：Speculative Decoding、Continuous Batching、Prefix Cache、推理压测。
- **RAG 深化**：Embedding 模型、向量数据库、rerank、GraphRAG、RAG 排障。
- **Agent 工程化**：workflow vs agent、工具设计、memory、HITL、observability、agent eval。
