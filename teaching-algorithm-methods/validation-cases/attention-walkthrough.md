...
> Attention 最容易被讲得很吓人，因为一上来就是 Q、K、V 和矩阵乘法。我们先把公式放一边。你先想：一句话里出现了“它”，人为什么知道“它”指谁？因为你会回头看前文，找最相关的对象。Attention 做的事，直觉上就是这个。
...
> 现在再看公式就没那么冷冰冰了。它其实就是把“我想找什么”和“谁能匹配我”算个分数，再按分数去拿内容。
...
> 这里有个很常见的误会：Attention 能看全局，不代表它没有代价。它看得越长，要维护的相关性矩阵就越大。所以长上下文模型才会反复讨论 FlashAttention、稀疏注意力这些优化。
...
> 所以面试里讲 Attention，不要只背公式。你要先讲它为什么需要，再讲 Q/K/V 各自扮演什么角色，最后补一句：它强在动态取信息，贵在长序列成本。
