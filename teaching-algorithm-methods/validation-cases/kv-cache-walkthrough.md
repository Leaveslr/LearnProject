...
> 我们先别急着背 KV Cache 的定义。你先想一个很日常的场景：你写一篇文章，已经写了前两句，接下来要写第三句。你会不会每写一个字，都把前两句重新整理一遍？正常人不会，对吧？但如果模型每生成一个新 token 都重新计算前文 K/V，本质上就在做这种重复劳动。
...
> 到这里先停一下。你要抓住一个点：KV Cache 不是让模型不看历史，而是让历史别每次都重新准备一遍。
...
> 这里很多人会问：那为什么不把 Q 也缓存？你可以这么理解，历史 K/V 像已经整理好的资料，会被后面的 token 一直查；但 Q 是“当前这一步我要问什么”，每一步的提问对象都不一样，所以它不是这个缓存机制的主角。
...
> 所以你面试时别只说“KV Cache 加速推理”。这句话太薄了。更像一个懂工程的人会说：它省掉历史 K/V 的重复计算，但把压力转移到了显存和缓存管理上。
